...
from app.models import StudentFine, StudentFineResponse, StudentFineUpdate, UserRole
...
async def create_student_fine(student_fine: StudentFine, db=Depends(get_db)):
    ...
    if student_fine_dict["student_id"] != issued_record.get("student_id"):
        raise HTTPException(
            status_code=400, detail="Student ID does not match the issued book record"
        )
    ...
    if return_date_raw:
        try:
            ...
            return_date = (
                return_date_raw
                if isinstance(return_date_raw, datetime)
                else datetime.fromisoformat(return_date_raw)
            )
        except Exception:
            raise HTTPException(status_code=400, detail="Invalid return_date format")
    else:
        raise HTTPException(
            status_code=400, detail="Missing return_date in issued record"
        )
    ...
    if now > return_date and not is_returned:
        ...
        raise HTTPException(
            status_code=400, detail="Book is not overdue or already returned"
        )
...
@router.get(
    "/",
    response_model=List[StudentFineResponse],
    dependencies=[Depends(require_roles(UserRole.librarian))],
)
...
async def update_student_fine(
    fine_id: str, fine_data: StudentFineUpdate, db=Depends(get_db)
):
    student_fine_collection = db["student_fine_collection"]
    try:
        obj_id = ObjectId(fine_id)
    ...
    result = await student_fine_collection.update_one({"_id": obj_id}, {"$set": update})
